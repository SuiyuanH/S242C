...
feature_idxs = [23, 39, 273, 52, 36, 144, 136, 24, 26, 125]
...
savepath = 'Results/Specific_features_0726'
...
for domain in domains:
    ...
    X = data_split [[f'feature_{i}' for i in feature_idxs]]
    ...
    # 使用训练集和全部特征，对数据进行二分类为['human','machine']，模型使用xgboost
    Xv = data_split [[f'feature_{i}' for i in feature_idxs]]
    ...
    X = sub_data [[f'feature_{i}' for i in feature_idxs]]
    ...
    for frank in range (len (feature_idxs)):
        # fidx, fpsp, fpsn, fnsp, fnsn, zero, direction
        fidx = feature_idxs [shap_rank [frank]]
        shap_value = shap_values [:, shap_rank [frank]]
        feature_value = X.iloc [:, shap_rank [frank]].values
