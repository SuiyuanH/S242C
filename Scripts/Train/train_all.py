...
savepath = 'Results/Shap_all30_0725'
...
data_split = all_data[all_data['split'] == 'train']
...
# 通过'split'的取值划分训练集，验证集和测试集
data_split = all_data[all_data['split'] == 'valid']
...
#for domain in domains:
    ...
    #all_data = all_data[all_data['domain'] == domain].copy()
for split in splits:
    # 通过'split'的取值划分训练集，验证集和测试集
    data_split = all_data[all_data['split'] == split]
...
X = all_data [[f'feature_{i}' for i in range(end)]]
y = (all_data ['label'] == 'machine').astype (np.int32)

# 计算预测结果的概率和标签
all_data.loc [:, 'proba'] = model.predict_proba(X) [:, 1]
all_data.loc [:, 'pred_label'] = ['machine' if i else 'human' for i in model.predict(X).tolist ()]

# 计算整个domain下所有数据点的shap值
explainer = shap.Explainer(model)
shap_values = explainer.shap_values(X)
all_data.loc [:, [f'shap_{i}' for i in range(end)]] = shap_values
...
filled = np.zeros (301)
filled [:end] = shap_sum
results_df.loc[len(results_df)] = ['all'] + accs + filled.tolist ()

shap.summary_plot(shap_values, X, show=False)
plt.title(f'SHAP Summary Plot')
plt.savefig(f'{savepath}/shap_all.png')
plt.close()
...
for frank in range (num_f):
    ...
    shap_value_all = shap_values [:, fidx]
    feature_value_all = X.iloc [:, fidx].values
    for domain in domains:
        feature_value = feature_value_all [(all_data['domain'] == domain).values]
        ...
        for irank in range (5):
            ...
            case_df.loc [len (case_df)] = [domain, fidx, features_name.iloc [fidx, 0], frank, fpsp, fpsn, fnsp, fnsn, zero, direction, all_data.index [cidx], all_data.iloc [cidx, 0], all_data.iloc [cidx, 5], all_data.iloc [cidx, 7], all_data.iloc [cidx, 6], shap_value [cidx], feature_value [cidx], irank, all_data.iloc [cidx, 1]]
        for irank in range (5):
            cidx = shap_f_rank [-1 - irank]
            case_df.loc [len (case_df)] = [domain, fidx, features_name.iloc [fidx, 0], frank, fpsp, fpsn, fnsp, fnsn, zero, direction, all_data.index [cidx], all_data.iloc [cidx, 0], all_data.iloc [cidx, 5], all_data.iloc [cidx, 7], all_data.iloc [cidx, 6], shap_value [cidx], feature_value [cidx], -1 - irank, all_data.iloc [cidx, 1]]


# 将更新后的all_data保存到f'{savepath}/all_data_with_shap_0724.csv'
all_data.to_csv(f'{savepath}/all_data_with_shap_0725.csv', index=False, encoding='utf-8')

# 将记录表保存到f'{savepath}/all_domains_with_shap_0724.csv'
results_df.to_csv(f'{savepath}/all_domains_with_shap_0725.csv', index=False, encoding='utf-8')

case_df.to_csv(f'{savepath}/all_cases_0725.csv', index=False, encoding='utf-8')
